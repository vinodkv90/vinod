            ...
            <div className="flex flex-wrap gap-4">
                {
                    projects.map((project) => (
                        <div key={project.id} className={`group relative overflow-hidden rounded-sm ${project.isLarge ? 'w-full xs:w-[calc(50%-8px)] md:w-[calc(66.6666%-8px)]' : 'w-full xs:w-[calc(50%-8px)] md:w-[calc(33.3333%-8px)]'}`}>
                            <figure className='w-full relative overflow-hidden z-0 after:pb-40 after:block after:w-full bg-radial from-emerald-800 from-40% to-emerald-900'>
                                <Image src={project?.image ?? '/gradient.jpg'} alt={project?.description} fill className='transition-all group-hover:scale-90 group-hover:opacity-0 object-contain object-center max-w-[100px] max-h-[100px] !left-1/2 -translate-x-1/2 !top-1/2 -translate-y-1/2' />
                            </figure>
                            <div className="flex flex-col justify-end absolute inset-0 z-[1] opacity-0 group-hover:opacity-100 transition-all duration-300 bg-foreground p-4 bg-radial from-emerald-800 from-40% to-emerald-900">
                                <h3 className='text-background font-montserrat font-bold tracking-wide text-base'>{project.name}</h3>
